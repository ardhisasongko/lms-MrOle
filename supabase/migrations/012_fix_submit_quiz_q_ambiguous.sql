-- Migration 012: Fix "column reference q.id is ambiguous" in submit_quiz
-- Cause: PL/pgSQL variable `q` collides with the SQL range alias `q`
-- (JOIN questions q) in the quiz_answers INSERT..SELECT.
-- Rename the SQL alias to qq.
...
BEGIN
  ...
  SELECT v_attempt_id, (ans->>'question_id')::UUID, ans->>'user_answer',
         lower(trim(qq.correct_answer)) = lower(trim(ans->>'user_answer'))
  FROM jsonb_array_elements(p_answers) ans
  JOIN questions qq ON qq.id = (ans->>'question_id')::UUID;
